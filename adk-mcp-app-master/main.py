...
logging.info("Starting ADK MCP application with enhanced logging")
...
def create_agent(server_params: StdioServerParameters):
    ...
    """
    try:
        # Create MCP toolset with no authentication (for local development)
        mcp_toolset = MCPToolset(
            connection_params=server_params, 
            timeout=30.0,
            auth_config=None  # Explicitly set to None to suppress warnings
        )
        logging.info(f"MCP Toolset created successfully with timeout: 30.0s")
        root_agent = LlmAgent(
        ...
        logging.info("ADK Agent created successfully with MCP tools")
        return root_agent
    except Exception as e:
        logging.error(f"Failed to create agent with MCP tools: {e}")
        ...
        logging.warning("Created agent without MCP tools due to error")
        return root_agent
